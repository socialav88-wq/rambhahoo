...
export function generateLocalityMetadata(locality) {
  return generateMetadata({
    exactTitle: `${locality.name} Community & Local Discussions | Rambhahoo`,
    description: `Join the ${locality.name} community. Discover real-time local discussions, neighborhood updates, and ${locality.name} events on Rambhahoo.`,
    path: `/${locality.slug}`,
  });
}
...
export function generateBusinessMetadata(business, locality) {
  return generateMetadata({
    title: `${business.name} - ${business.category} in ${locality?.name || 'Hyderabad'}`,
    description: business.description || `Find reviews, photos, tags, and updates for ${business.name} in ${locality?.name || 'Hyderabad'} on Rambhahoo.`,
